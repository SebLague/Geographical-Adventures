...
public class AtmosphereEffect : PostProcessingEffect
{
	...
	public void SetProperties()
	{
		...
		if (light == null)
		{
			light = GameObject.FindGameObjectWithTag("Sun").GetComponent<Light>();
		}
	...
	// Set textures/values on the atmosphere shader.
	// Note: this doesn't include the 'shared' values, which are handled in GetShaderValues()
	void SetAtmosphereShaderParams()
	{
		// Textures
		material.SetTexture("Sky", sky);
		material.SetTexture("TransmittanceLUT", transmittanceLUT);
		material.SetTexture("AerialPerspectiveLUT", aerialPerspectiveLuminance);
		material.SetTexture("TransmittanceLUT3D", aerialPerspectiveTransmittance);
		...
		material.SetFloat("aerialPerspectiveStrength", aerialPerspectiveStrength);

		material.SetFloat("intensity", intensity);
		material.SetFloat("contrast", contrast);
		material.SetFloat("whitePoint", whitePoint);
		material.SetFloat("skyTransmittanceWeight", skyTransmittanceWeight);
	}
	...
	void RenderSky()
	{
		SetRaymarchParams(Camera.current, skyRenderCompute);
