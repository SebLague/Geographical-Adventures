...
public class IcoSphere
{
	...
	public Mesh GetMesh(float radius = 1)
	{
		Mesh mesh = new Mesh();
		...
		{
			scaledVerts[i] = Vertices[i] * radius;
		}

		mesh.SetVertices(scaledVerts);
		mesh.SetTriangles(Triangles, 0, true);
		mesh.RecalculateNormals();
		mesh.UploadMeshData(markNoLongerReadable: true);
		mesh.name = $"IcoSphere (verts={Vertices.Length}, tris={Triangles.Length})";
		return mesh;
	}
