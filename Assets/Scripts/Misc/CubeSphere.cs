...
public static class CubeSphere
{
	...
	public static MeshData[] GenerateMeshes(int resolution, int numSubdivisions = 1)
	{
		...
		foreach (Vector3 faceNormal in faceNormals)
		{
			for (int y = 0; y < numSubdivisions; y++)
			{
				for (int x = 0; x < numSubdivisions; x++)
				{
					...
					meshes[meshIndex] = CreateFace(resolution, faceNormal, startT, endT);
					meshIndex++;
				}
			}
		}
	...
	static MeshData CreateFace(int resolution, Vector3 normal, Vector2 startT, Vector2 endT)
	{
